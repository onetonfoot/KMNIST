...
# Tweaked from here
# https://github.com/NegatioN/OnlineMiningTripletLoss/blob/master/triplet_loss.py
# Explaing blog post can be found here
# https://gombru.github.io/2019/04/03/ranking_loss/


def batch_hard_triplet_loss(labels, embeddings, margin=2.0, squared=False):
...
def batch_all_triplet_loss(labels, embeddings, margin=2.0, squared=False):
    """Build the triplet loss over a batch of embeddings.
...
def _get_anchor_positive_triplet_mask(labels):
    ...
    indices_equal = torch.eye(labels.size(
        0), device=device).byte()
    indices_not_equal = ~indices_equal
